...
需要围绕特定主题生成帖子时（比如某场具体比赛的报名攻略、某品牌装备对比、近期赛季建议等），可以在生成时附加自定义说明。

![自定义帖子输入框](image-16.png)

1. 选择帖子类型（决定使用哪组标签和评论）
2. 在输入框中用中文写清楚你想要的方向，例如：
   - `围绕大阪马拉松2026，重点讲一下报名流程和注意事项`
   - `写一篇比较佳明和苹果手表的内容，面向准备跑第一场全马的新手`
3. 点击 **生成帖子**

> **注意：** 选择"赛事"类型时，Claude 会自动参考当前数据库中所有赛事信息，无需手动指定比赛。如果不填写自定义说明，系统会自动从未发过的赛事中随机选一场生成。
...
系统会记录最近 7 天内已发过帖子的赛事，生成时自动跳过这些赛事，避免重复。

左侧"**赛事冷却**"区域显示当前在冷却期内的赛事列表。如果某场比赛处于冷却期但你想再次发帖，可以手动重置：

1. 展开"赛事冷却"面板
2. 找到对应赛事，点击 **重置**
3. 该赛事从冷却列表移除，下次生成时可重新被选中

> 7 天后系统会自动清除冷却记录，通常不需要手动操作。
...
> **提示：** 不上传文件也可以直接点击 **分析**——系统会用数据库中已有的历史数据分析，适合不想每次都导出文件的情况。
