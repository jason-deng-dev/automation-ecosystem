...
**最后更新：** 2026 年 5 月
**服务器：** AWS Lightsail — `13.192.170.85`（用户名：`ubuntu`）
...
1. **打开面板**，地址：`http://13.192.170.85:3002`
...
五个 Docker 容器运行在同一台 AWS Lightsail 实例（`13.192.170.85`）上，共同驱动：
...
| SSH 命令 | `ssh -i ~/.ssh/automation-ecosystem.pem ubuntu@13.192.170.85` |
...
**修复步骤：**
1. 从对应服务商获取新密钥
2. SSH 登录服务器：`ssh -i ~/.ssh/automation-ecosystem.pem ubuntu@13.192.170.85`
