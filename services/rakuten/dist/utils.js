...
function cleanDescription(raw) {
    ...
    if (probeLen > 5) {
        ...
        if (dupeStart > probeLen)
            text = text.slice(0, dupeStart).trim();
    }
    ...
    text = sentences.filter(s => /[■◇●◆・]/.test(s) || !boilerplatePatterns.some(re => re.test(s))).join('。').trim();
...
function handleInlineSpecFormat(text) {
...
function extractShortDescription(raw) {
    if (!raw)
        return '';
    let text = raw;
...
async function translateNames(normalizedItems) {
    ...
    return normalizedItems.map((item, i) => (Object.assign(Object.assign({}, item), { itemName: results[i].text })));
}
function normalizeItems(items) {
    return items.map(({ Item: { itemName, itemPrice, itemCaption, itemUrl, smallImageUrls, mediumImageUrls, reviewCount, reviewAverage, shopName, shopCode, genreId, availability, }, }) => ({
        itemName,
    ...
    return [
        {
            Item: {
                ...
                mediumImageUrls: [
                    {
                        imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/alpen/cabinet/img/757/4303565814_8.jpg?_ex=128x128",
                    },
                    {
                        imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/alpen/cabinet/img/757/4303565814_1.jpg?_ex=128x128",
                    },
                    {
                        imageUrl: "https://thumbnail.image.rakuten.co.jp/@0_mall/alpen/cabinet/img/757/4303565814_2.jpg?_ex=128x128",
                    },
                ],
                affiliateUrl: "",
                shopAffiliateUrl: "",
        ...
        {
            Item: {
                itemName: "【セール】サッカニー Saucony SPECIAL PRICE メンズ KINVARA 15 キンバラ15 ランニング マラソン ジョギング トレーニング ジム フィットネス スポーツ 運動靴 日常ラン 負担軽減 サポート S20967",
                catchcopy: "1898年創業 100年以上にわたりランニングに携わってきた私たちは、ボストン郊外の地元の工場からグローバルブランドへと成長し、世界中でランニングの楽しさを発信しています",
                ...
                itemPriceMin2: 8800,
                itemPriceMin3: 8800,
                itemCaption: "商品情報 ITEM DETAILS まるで羽のように軽い履き心地。KINVARA 15は、柔軟性とクッション性を兼ね備えた、デイリーランに最適なランニングシューズです。軽量で無駄のないデザインに、PWRRUNクッションとSRSソックライナーを搭載し、長距離でも快適に走れるサポート力を発揮します。",
