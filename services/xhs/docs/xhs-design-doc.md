...
**Dashboard → XHS script invocation:** The Dashboard server has access to the Docker socket and invokes XHS scripts via `docker exec` — no HTTP server is needed inside the XHS container. All operator-triggered actions (manual post, preview, re-auth) are exposed as scripts that the Dashboard runs directly:
...
- ~~**Human review gate:** Should v1 include an optional manual review step before publishing, to validate Claude output quality before going fully autonomous?~~ **Resolved:** Dashboard manual trigger supports `preview: true` — generates and archives the post without publishing. Operator can review from post archive before going fully autonomous.
